...
class Face_Recognition:
    def __init__(self, root):
        ...
        img_top1 = Image.open(r"college_images\face_detector1.jpg")
        img_top1 = img_top1.resize((650, 700), Image.LANCZOS)
        self.photoimg_top1 = ImageTk.PhotoImage(img_top1)

        f_lbl1 = Label(self.root, image=self.photoimg_top1)
        f_lbl1.place(x=0, y=55, width=650, height=700)

        img_top2 = Image.open(r"college_images\facial_recognition_system_identification_digital_id_security_scanning_thinkstock_858236252_3x3-100740902-large.jpg")
        ...
        b1_1 = Button(f_lbl2, text="Face Recognition", cursor="hand2", command=self.face_recog,
    ...
    def face_recog(self):
        def draw_boundray(img, classifier, scaleFactor, minNeighbors, color, text, recognizer):
            greay_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            features = classifier.detectMultiScale(greay_image, scaleFactor, minNeighbors)

            coord = []

            for (x, y, w, h) in features:
                cv2.rectangle(img, (x, y), (x + w, y + h), (0,255, 0), 3)
                id, predict = recognizer.predict(greay_image[y:y + h, x:x + w])
                confidence = int(100 * (1 - predict / 300))

                conn = mysql.connector.connect(host="localhost", username="root", password="2407",
